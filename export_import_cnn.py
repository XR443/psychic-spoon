...
def import_model(model_type):
    ...
    model_path = Path(f"./models/{model_type.__name__}.pt")
    if model_path.exists():
        return torch.load(model_path, weights_only=False)
    return mlflow.pytorch.load_model(f"models:/{model_type.__name__}/latest")
...
if __name__ == "__main__":
    model_type = sys.argv[1] if len(sys.argv)>1 else "EncoderDecoder"
